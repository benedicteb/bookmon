...
fn test_book_id_matches_storage_key() {
    ...
    let stored_book = storage.books.get(&book.id).expect("Book should exist in storage");
    assert_eq!(stored_book.id, book.id, "Book ID should match its storage key");
}
...
fn test_book_timestamp_format() {
    ...
    let timestamp_str = value["added_on"].as_str().expect("added_on should be a string");
